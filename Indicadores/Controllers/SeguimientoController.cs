        ...
        public ActionResult Get(int id,short idAgen)
        {
            try
            {
               var item =(dynamic) null;
                if (id == 391)
                {
                    item = _context.SeguimientoClass.FromSqlRaw($"Select S.Id, S.Cod_Prod, P.Nombre as Productor, S.Cod_Campo, C.Descripcion as Campo, A.IdAgen, A.Nombre as Asesor, (case when S.Estatus='A' THEN 'ATENCIÓN A PRODUCTORES' else case when S.Estatus='M' THEN 'CIERRE DE MATERIAL' else case when S.Estatus='C' THEN 'COBRANZA' else case when S.Estatus = 'R' THEN 'PENDIENTE REVISIÓN' else case when S.Estatus = 'G' THEN 'REVISA GERENCIA' else case when S.Estatus = 'S' THEN 'SALDADO' else case when S.Estatus = 'T' THEN 'TERMINO TEMPORADA' else case when S.Estatus = 'E' THEN 'VA A ENTREGAR' else case when S.Estatus = 'P' THEN 'VA A PAGAR' else '' end end end end end end end end end)  as Estatus,(case when S.Comentarios is null then '' else S.Comentarios end) as Comentarios, convert(varchar, S.Fecha, 23) as Fecha, (case when S.Estatus is null then DATEDIFF(day, S.Fecha, getdate()) else '' end) as dias,round(isnull(R.cjs1, 0), 0) as caja1, round(isnull(R2.cjs2, 0), 0) as caja2, CONVERT(varchar, CONVERT(money, round(isnull(F.SaldoFinal, 0), 0)), 1) as SaldoFinal, isnull(SE.Semana, 0) as Semana, S.AP, S.Fecha_Up, S.IdAgen as IdAgenS, A.IdRegion  " +
